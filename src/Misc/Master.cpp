...
void Master::AudioOut(float *outl, float *outr)
{
    ...
    for(int nefx = 0; nefx < NUM_SYS_EFX; ++nefx) {
        ...
        std::vector<float> tmpmixl(synth->buffersize);
        std::vector<float> tmpmixr(synth->buffersize);
        //Clean up the samples used by the system effects
        memset(tmpmixl.data(), 0, synth->bufferbytes);
        memset(tmpmixr.data(), 0, synth->bufferbytes);
        ...
        sysefx[nefx]->out(tmpmixl.data(), tmpmixr.data());
